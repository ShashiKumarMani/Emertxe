...
function arithmetic_operation()
{
     ...
     do  
	     ...
	     #case statement to perform division seperately (if doesnt work)
	     case $operator in
		 "/")
		     result=$(echo "scale=2; $result$operator${string:${#string}-i:1}" | bc)
		     ;;
		 *)
		     # result = (result (operator) string[n] )
		     result=$(echo "$result$operator${string:${#string}-i:1}" | bc)
		     ;;
	     esac
         fi
     done
